...
void printHelp(const char * pname, ::ofstream &fd) {
  ...
  ArgParseUtils::displayOption(fd, "--coverage=PERCENT", "Minimum percentage exon coverage for an exon to be declared correct. It's only used for reporting exon accuracy numbers [100]");
...
int main(int argc, char *argv[]) {
  ...
  int maxIterations = 100,
    ...
    minLongerWait = 1;
  int kBest = 1, i, noiseLevel = 0, blockSize = 1, num_sigfilters = 4;
  ...
  TLossType elf = LF_NONE, lf = LF_HAMMING;
  ...
  try {
    ...
    for(i = 1; i < argc; i++) {
      if(!strncmp(argv[i], "--resources=", 12)) {
	...
	prefix_evidence = string(argv[i] + 18);
      ...
      else if(!strncmp(argv[i], "--learn-rate=", 13)) {
	...
	addUnreachable = true;
        ...
        oracleUpd = TypeDefs::stringToTOracleUpd(oracleUpdString);
              ...
              || !strncmp(argv[i], "-h", 2)) {
        printHelp("train_craig", (std::ofstream &)cout); 
        exit(0);  
      }
    ...
    if(strand.compare("forward") == 0)
      ...
      throw EXCEPTION(BAD_USAGE, std::string("unrecognized argument") + strand);
    
    confFile = std::string(argv[i++]);
    paramFile = std::string(argv[i++]);
    
    cerr << "Training model for configuration " << confFile << endl;
    cerr << "Options :";
    for(i = 1; i < argc; i++)
      cerr << " " <<  argv[i];
    cerr << endl;
    ...
    string &prefix = (prefix_evidence.length() ? prefix_evidence :
		      config.prefixFiles());
    
    map<std::string, std::string> resource_subs;
    resource_subs["PREFIX_EVIDENCE"] = prefix;
    ...
    MPStructureCore core(fsm, lattice, re, fe, fte, 
                         ...
                         maxLoss, multiUpd, oracleWait, oracleUpd,
			 addUnreachable);
    ...
    if(gle->error() == BAD_USAGE)
      cerr << "use -h, --help for more information\n";
    delete e;
    exit(1);

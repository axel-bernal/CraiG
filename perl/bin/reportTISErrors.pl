...
my $ds = 0;
my $ups = 0;
my $usage = "usage : $0 annotations predictions start_score\n";

my $annotFile = shift || die $usage;
my $predFile = shift || die $usage;
my $scoreFile = shift || die $usage;

open(ANNOT, "$annotFile") || die $usage;
open(PRED, "$predFile") || die $usage;
open(SCORE, "$scoreFile") || die $usage;

my %agenes = % {&GenUtils::readLocsFormat(\*ANNOT)};
my %pgenes = % {&GenUtils::readLocsFormat(\*PRED)};
...
foreach my $id (keys %pgenes) {
    ...
    my $start = $gene->{'Exons'}->[0]->[1];
    $starts{$cid}->{$start} = $gene;
}

foreach my $id (keys %agenes) {
    my $gene = $agenes{$id};
    my $cid = $agenes{$id}->{'Exons'}->[0]->[0];
    my $start = $gene->{'Exons'}->[0]->[1];

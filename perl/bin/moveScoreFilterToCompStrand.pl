...
#	    print "$strand $i $f_vals1->[$i] $f_vals2->[$i]\n";
	my $score_val = $f_vals->[STRAND_FWD]->[$i];
	$f_vals->[STRAND_FWD]->[$i] = 0;
	$f_vals->[STRAND_COMP]->[$clen - $i + 1] += $score_val;
    }	

    GenUtils::printContigScoreFilterVals($cid, \%fVals, \*STDOUT, STRAND_COMP);
    %fVals = ();

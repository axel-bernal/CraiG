...
sub make_graphs{
    ...
    my $bin_num = 1;
    $bin_size = 487; 
    my $bin_stop = int($graph_min + exp($bin_num*log(2.0))*$bin_size);
#    my $bin_stop = 3899;
	...
	$bin_num++;
	$bin_stop = int($graph_min + exp($bin_num*log(2.0))*$bin_size);
#        if($bin_stop < 17582) {
#            $bin_stop = 17582;
#        }
#        elsif($bin_stop < 131693) {
#            $bin_stop = 131693;
#        }
#	elsif($bin_stop < $graph_max) {
#            $bin_stop = $graph_max;
#        }
#        else {
#            $bin_stop++;
...
sub compare_overlapping_introns{
	...
	$$pt{Match5} = 1;
	$$pt{Match3} = 1;
#        print ">",$p->{Transcript}->{Id},"\t", $p->{Transcript}->{Gene}->{Seqname}."_".$p->start."_".$p->stop."\n";
    }
    else{
	if($a->strand eq '+'){
	    if($lm){
